/**
 * 🚨 [절대 엄금 개발 규칙 / STRICT CODE DEVELOPMENT RULE]
 * 1. 기존 시트 데이터나 헤더(1행)를 절대로 덮어쓰거나 수정/삭제하지 말 것.
 * 2. 재고조사 등 타 목적 작업 시, 대여 현황 시트('시트1')나 타 시트로 임의 폴백(Fallback)하여 덮어쓰는 행위 엄격 금지.
 * 3. 지정된 시트 탭(GID)을 찾지 못할 경우, 다른 시트를 건드리지 말고 반드시 에러(createErrorResponse)를 반환하고 작업을 안전하게 중단할 것.
 */
